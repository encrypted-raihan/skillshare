...
create unique index if not exists conversations_pair_unique_idx on public.conversations (least(user_a, user_b), greatest(user_a, user_b));
...
alter table public.messages add constraint messages_message_type_check check (message_type in ('text','image','document','audio'));
...
create policy conversations_select_participant on public.conversations for select to authenticated using (user_a = auth.uid() or user_b = auth.uid());

drop policy if exists conversations_insert_participant on public.conversations;
create policy conversations_insert_participant on public.conversations for insert to authenticated with check (user_a = auth.uid() or user_b = auth.uid());

drop policy if exists conversations_update_participant on public.conversations;
create policy conversations_update_participant on public.conversations for update to authenticated using (user_a = auth.uid() or user_b = auth.uid()) with check (user_a = auth.uid() or user_b = auth.uid());
...
create policy messages_select_participant on public.messages for select to authenticated using (
  ...
  and not exists (select 1 from public.message_deletions md where md.message_id = id and md.user_id = auth.uid())
);
...
create policy message_deletions_select_self on public.message_deletions for select to authenticated using (user_id = auth.uid());

drop policy if exists message_deletions_insert_self on public.message_deletions;
create policy message_deletions_insert_self on public.message_deletions for insert to authenticated with check (
...
create policy message_media_insert on storage.objects for insert to authenticated with check (
...
create policy message_media_select on storage.objects for select to authenticated using (
  bucket_id = 'message-media' and (
    (storage.foldername(name))[1] = auth.uid()::text
    or exists (
      select 1 from public.messages m join public.conversations c on c.id = m.conversation_id
      where m.media_path = name and (c.user_a = auth.uid() or c.user_b = auth.uid())
    )
  )
);

drop policy if exists message_media_delete on storage.objects;
create policy message_media_delete on storage.objects for delete to authenticated using (
  bucket_id = 'message-media' and (storage.foldername(name))[1] = auth.uid()::text
);
